  ...
  public  class NewExeclHelper
    {
        ...
        public static DataTable ImportExceltoDt(string strFileName, List<ColumnCharts> liListColumnst, int HeaderRowIndex)
        {
            ...
            dt = ImportDtColumn(sheet, HeaderRowIndex, true, liListColumnst);
            return dt;
        }
        ...
        /// <param name="HeaderRowIndex">列头所在行号，-1表示没有列头</param>
        /// <returns>DataTable.</returns>
        public static DataTable ImportExceltoDt(string strFileName, int SheetIndex, int HeaderRowIndex,DataTable table )
        {
            HSSFWorkbook workbook;
            IWorkbook wb;
            using (FileStream file = new FileStream(strFileName, FileMode.Open, FileAccess.Read))
...
//            DataTable table = new DataTable();
            table = ImportDt(isheet, HeaderRowIndex, true, table);
            //ExcelFileStream.Close();
            workbook = null;
            isheet = null;
            return table;
        ...
        static Hashtable ImportDtColumns(ISheet sheet, int HeaderRowIndex, bool needHeader,  List<CustomEntity> outputlist)
        {
        ...
        /// <param name="needHeader">if set to <c>true</c> [need header].</param>
        /// <returns>DataTable.</returns>
        static DataTable ImportDt(ISheet sheet, int HeaderRowIndex, bool needHeader, DataTable table)
        {
//            DataTable table = new DataTable();
            IRow headerRow;
            int cellCount;
        ...
        /// <param name="CategoryTableID">选择的模板表格id</param>
        /// <returns></returns>
        static DataTable ImportDtColumn(ISheet sheet, int HeaderRowIndex, bool needHeader, List<ColumnCharts> liListColumnst)
        {
            DataTable table = new DataTable();
            IRow headerRow;
            int cellCount;
            try
            {
                ...
                else
                {
                    ...
                    for (int i = headerRow.FirstCellNum; i <cellCount; i++)
                    {
                        int rownum = 0;
                        int cellnum = 0;
                        if (IsMergedRegionCell(i, HeaderRowIndex, sheet, ref rownum,ref cellnum))
                        {
                              string Browvalue = stringcellvalue(sheet.GetRow(rownum).GetCell(cellnum));
                          var columncmodel=  liListColumnst.FirstOrDefault(t => t.title.Equals(Browvalue));

                          if (columncmodel!=null)
                            {
                                DataColumn column = new DataColumn(columncmodel.field);
                                table.Columns.Add(column); 
                            }
                        ...
                        else
                        {
                            ...
                            var columncmodel = liListColumnst.FirstOrDefault(t => t.title.Equals(headerRow.GetCell(i).ToString()));
                        ...
                        //    if (headerRow.GetCell(i).ColumnIndex >= cellrange.FirstColumn && headerRow.GetCell(i).ColumnIndex <= cellrange.LastColumn
                        //        && headerRow.GetCell(i).RowIndex >= cellrange.FirstRow && headerRow.GetCell(i).RowIndex <= cellrange.LastRow)
                        //    {
        ...
        // 判断单元格是否被合并
        public static bool IsMergedRegionCell(int cellIndex, int rowIndex, ISheet sheet, ref int firstRegionRow,ref int firsRegionCell)
        {
            bool isMerged = false;
                    ...
                    if (rowIndex == i)
                    {
                        for (int j = cellRangeAddress.FirstColumn; j <= cellRangeAddress.LastColumn; j++)
                        {
                            if (cellIndex == j)
                            {
                                ...
                                firsRegionCell = cellRangeAddress.FirstColumn;
                                break;
                            }
                            else
                            {
        ...
        static string stringcellvalue(ICell cle)
        {
            #region
            ...
            if (cle != null)
            {
                ...
                {
                    case CellType.String:
                        string str = cle.StringCellValue;
                        if (str != null && str.Length > 0)
                        {
                            returnstrvalue = str;
                        }
                        break;
                    case CellType.Numeric:
                        if (DateUtil.IsCellDateFormatted(cle))
                        {
                            returnstrvalue =
                                    DateTime.FromOADate(cle.NumericCellValue).ToString();
                        }
                        else
                        {
                            returnstrvalue =
                                    Convert.ToDouble(cle.NumericCellValue).ToString();
                        }
                        break;
                    case CellType.Boolean:
                        returnstrvalue =
                                Convert.ToString(cle.BooleanCellValue).ToString();
                        break;
                    case CellType.Error:
                        returnstrvalue = "";
                        break;
                    case CellType.Formula:
                        switch (cle.CachedFormulaResultType)
                        {
                            case CellType.String:
                                string strFORMULA = cle.StringCellValue;
                                if (strFORMULA != null && strFORMULA.Length > 0)
                                {
                                    returnstrvalue = strFORMULA.ToString();
                                }
                                else
                                {
                                    // dataRow[j] = null;
                            ...
                            case CellType.Numeric:
                                returnstrvalue =
                                        Convert.ToString(
                                            cle.NumericCellValue);
                                break;
                            case CellType.Boolean:
                                returnstrvalue =
                                        Convert.ToString(
                                            cle.BooleanCellValue);
                                break;
                            case CellType.Error:
                                returnstrvalue =
                                        ErrorEval.GetText(cle.ErrorCellValue);
                                break;
                            default:
